...
def channel_rescaled(input_channel, min_val, max_val, th_low_input, th_high_input):    
	...
	#Rescaling
	image_rs3 = (input_channel - th_low_input) * (max_val / (th_high_input - th_low_input)) 
    
    #Capping
	image_rs3 = (image_rs3 > max_val).ifthenelse(max_val, image_rs3) #true, false
	output_channel = (image_rs3 < min_val).ifthenelse(min_val, image_rs3)

    #uint8    
	# output_channel = image_rs3.cast("uchar") 
...
def stitch_crop_rescale(fileList2, tiles_across, outPct, outputFolder):    
    ...
    image_rescaled = img_rescaled(image_cropped, outPct)
...
def build_pyramids(chosen_table, type_list, tileSize, filterSize, pctOut, destDir, save_recoloured, flip_ud):
    ...
    for filename in fileList0:
        ...
        item = []
        item.extend(statistic_vals) #original   

        #Transforming input		
        for type in type_list:

            if type == "log":  #natural log() 
                image_positive = image - statistic_vals[0] + 1
            ...
            item.extend(array3) #for table             

            # #Standardising z-score (for UMAP and PCA, not DSA)           
            # image_zscore = channel_standardise(image_med, array3[4], array3[5])
            ...
            #For stacks
            if type == "log":
                log_list.append(image_med) #image_zscore                    
            elif type == "linear":
                linear_list.append(image_med)	             
        ...
        stats_list3.columns = stats_cols2                        
    ...
    if "linear" in type_list: 
        save_transformed_stack(linear_list, 'linear', tileSize, flip_ud, destDir)

    if "log" in type_list:
        save_transformed_stack(log_list, 'log', tileSize, flip_ud, destDir) 

    if "original" in type_list: 
        save_stack(fileList0, tileSize, flip_ud, destDir)

    metadata = learn_tileConfiguration(destDir)
