...
The package to process rock chemical map pixels into dimensionally reduced representation images (flat RGB TIFs) that maximise the original images content of any microscopy and micro-analysis technique. It features the unsupervised learning analysis of full resolution micro-analysis maps (e.g., intensity from X-ray lines, mass spectrometry analytes) using linear and non-linear dimensionality reduction such as Principal component analysis (PCA), the Uniform Manifold Approximation and Projection (UMAP), and the Deep sparse autoencoder (DSA) neural network. 

It includes an image alignment options require control points that have been placed manually using ImageJ BigWarp plugin ([Bogovic et al., 2016](https://imagej.net/plugins/bigwarp)). The plugin allows exporting a 'landmarks.csv' file containing the ID and locations (X, Y) of the placemarks accross the moving and fixed images (at least 4 points are required to fit the transform models). This repository contains an example CSV to show the user for required format (see 'landmarks_bse_xpl.csv').
...
The data-driven approach focuses on the reduction process parametrisation and lesens the focus on interpreting the input images. With trialling and well-characterised samples, I demonstrated that:
